resource "google_service_account" "core_service_account" {
  account_id   = "core-service"
  display_name = "Service account for core application"
  depends_on   = [google_project_service.project_services]
}

resource "google_project_iam_member" "core_service_account_iam" {
...
resource "kubernetes_service_account" "core_service_account" {
  depends_on = [module.gke]
  provider   = kubernetes.gke

  metadata {
    name      = google_service_account.core_service_account.account_id
    namespace = var.core_namespace

    annotations = {
      "iam.gke.io/gcp-service-account" = google_service_account.core_service_account.email
    }
  }
}
...
resource "google_service_account_iam_member" "core_service_account" {
  service_account_id = google_service_account.core_service_account.name
  role               = "roles/iam.workloadIdentityUser"
  member             = "serviceAccount:${var.project_id}.svc.id.goog[${var.core_namespace}/${google_service_account.core_service_account.account_id}]"
}