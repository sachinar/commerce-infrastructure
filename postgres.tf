...
resource "random_string" "wallet_user_password" {
  length           = 16
  special          = true
  override_special = "@#%&*()-_=+[]{}<>:?"
}
...
# wallet
resource "kubernetes_secret" "wallet_app_secret" {
  provider = kubernetes.gke
  metadata {
    name      = var.wallet_secret_name
    namespace = var.payment_namespace
  }

  data = {
    DB_HOST     = google_dns_record_set.inventory_postgres_a_record.name
    DB_NAME     = var.wallet_database
    DB_PORT     = "5432"
    DB_USER     = "wallet-user"
